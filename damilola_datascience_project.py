...
def my_model_line_chart():
    ...
    # This is to avoid the xlabel being to close to each other. So when the xlabel gets more, they would be slanted on display
    if len(standard_dates) > 190:
...
def my_model_predictions():
    ...
    while (timer1 <= 1):
        ...
        elif 1001 <= fetched_cases_total <= 3000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 800:
        ...
        elif 3001 <= fetched_cases_total <= 5000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 1200:
        ...
        elif 5001 <= fetched_cases_total <= 10000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 3000:
        ...
        elif 10001 <= fetched_cases_total <= 20000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 6000:
        ...
        elif 20001 <= fetched_cases_total <= 30000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 9000:
        ...
        elif 30001 <= fetched_cases_total <= 50000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 13000:
        ...
        elif 50001 <= fetched_cases_total <= 100000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 25000:
        ...
        elif 100001 <= fetched_cases_total <= 200000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 60000:
        ...
        elif 200001 <= fetched_cases_total <= 500000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 140000:
        ...
        elif 500001 <= fetched_cases_total <= 1000000:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 100000:
        ...
        elif fetched_cases_total > 1000001:
            index_iterator = 0
            zip_column_row = zip(final_cases_df.columns, final_cases_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 150000:
    ...
    while (timer2 <= 1):
        fetched_recoveries_total = new_recoveries_df.iloc[-1]
        if 0 <= fetched_recoveries_total <= 500:
            index_iterator = 0
        ...
        elif 501 <= fetched_recoveries_total <= 1000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 300:
        ...
        elif 1001 <= fetched_recoveries_total <= 3000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 800:
        ...
        elif 3001 <= fetched_recoveries_total <= 5000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 1500:
        ...
        elif 5001 <= fetched_recoveries_total <= 10000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 3000:
        ...
        elif 10001 <= fetched_recoveries_total <= 20000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 6000:
        ...
        elif 20001 <= fetched_recoveries_total <= 30000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 9000:
        ...
        elif 30001 <= fetched_recoveries_total <= 50000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 11000:
        ...
        elif 50001 <= fetched_recoveries_total <= 100000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 19000:
        ...
        elif 100001 <= fetched_recoveries_total <= 500000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 40000:
        ...
        elif 500001 <= fetched_recoveries_total <= 1000000:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 90000:
        ...
        elif fetched_recoveries_total > 1000001:
            index_iterator = 0
            zip_column_row = zip(final_recoveries_df.columns, final_recoveries_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 150000:
    ...
    while (timer3 <= 1):
        ...
        elif 501 <= fetched_deaths_total <= 1000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 300:
        ...
        elif 1001 <= fetched_deaths_total <= 5000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 800:
        ...
        elif 0 <= fetched_deaths_total <= 5000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 500:
        ...
        elif 5001 <= fetched_deaths_total <= 10000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 2000:
        ...
        elif 10001 <= fetched_deaths_total <= 20000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 4000:
        ...
        elif 20001 <= fetched_deaths_total <= 30000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 8000:
        ...
        elif 30001 <= fetched_deaths_total <= 50000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 9000:
        ...
        elif 50001 <= fetched_deaths_total <= 100000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 15000:
        ...
        elif 200001 <= fetched_deaths_total <= 500000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 80000:
        ...
        elif 500001 <= fetched_deaths_total <= 1000000:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 100000:
        ...
        elif fetched_deaths_total > 1000001:
            index_iterator = 0
            zip_column_row = zip(final_deaths_df.columns, final_deaths_df.iloc[index_iterator, :])
            for c,d in enumerate(zip_column_row):
                if d[1] >= 200000:
